...
class Register:
    ...
    def impact(self, root_question_id, node, node_kind, effect, detail=None):
        """Record something an answer would release.  Impacts are counted separately and never as questions."""
        self._impacts.append({"ROOT_QUESTION_ID": root_question_id, "NODE": node, "NODE_KIND": node_kind,
                              "EFFECT": effect, "DETAIL": detail or {}})

    def as_dict(self):
        roots = sorted(self._roots.values(), key=lambda r: (r["KIND"], r["SUBJECT_REF"]))
        impacts = sorted(self._impacts, key=lambda i: (i["ROOT_QUESTION_ID"], i["NODE_KIND"], str(i["NODE"])))
        by_kind = {}
        for r in roots:
