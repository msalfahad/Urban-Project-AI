...
def evaluate(*, registers, validation_result, review, benchmark_scan, tests_pass):
    ...
    rows = [
        ...
        ("BRIDGE_EMITS_NOTHING_FROM_UNRESOLVED_TOPOLOGY", not leaks and all(l["BARE_NUMBER"] is False for l in trace) and safety["BRIDGE_ALLOWED"] == sum(1 for r in safety["ROWS"] if not r["BLOCKED_BY"]),
         {"LEAKS": [l["LINE_ID"] for l in leaks], "BRIDGE_ALLOWED": safety["BRIDGE_ALLOWED"], "BLOCKED_BY_GATE": safety["BLOCKED_BY_GATE"]}),
        ("UNIT_GATE_PASSES", all(qa["UNITS_ACCEPTABLE"].values()) if qa["UNITS_ACCEPTABLE"] else False, qa["UNITS_ACCEPTABLE"]),
        ("SOURCE_AUDIT_PASSES", registers.get("PA07_SOURCE_AUDIT", {}).get("PASS") is True, registers.get("PA07_SOURCE_AUDIT", {})),
        ("FREEZE_REVERSIBILITY_PASSES", (tmr["REVERSIBILITY"]["ALL_REVERSIBLE"] in (True, None)) and (tmr["REVERSIBILITY"]["ALL_ZERO_MATERIAL"] in (True, None)) and passed("test_seals_close_every_interval_and_carry_no_material"),
         {"REGIONS": tmr["COUNT"], "REVERSIBILITY": tmr["REVERSIBILITY"]}),
