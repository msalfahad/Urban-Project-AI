"""The R7 engine, run over a real drawing, and graded by the independent gate.
...
from research.qs_wall_treatment_01.pa09.alrashed import regression_r7 as RG

OUT = Path(PR.OUT_DIR) / "pa09_alrashed"
FROZEN_SHA = "7e9a3eba636ea95b77fce2bbb7dccad79d6971047af455bb2f542ee62165493f"
REC = OUT / "ALRASHED_GENERIC_ENGINE_VALIDATION_R7.json"
...
def test_a_material_question_is_asked_about_a_group_and_never_about_a_wall_type(rec):
    """Asking per line reports fifty-four questions where the source has one gap.  Asking per thickness
    assumes an answer the source has not given: walls of one thickness may be built of different things."""
    q = register("ROOT_QUESTION_REGISTER")
    material = [r for r in q["ROOT_QUESTIONS"] if r["KIND"] == "WALL_MATERIAL_NOT_ESTABLISHED"]
    assert material
    for r in material:
        assert r["SUBJECT_KIND"] == "MATERIAL_SCOPE_GROUP"
        assert r["SUBJECT_REF"].startswith("MATERIAL_SCOPE::")
        assert not r["SUBJECT_REF"].startswith("THICKNESS_FAMILY::")
        assert r["DETAIL"]["ANSWER_PROPAGATES"].startswith("only as far as")
    scopes = register("MATERIAL_SCOPE_REGISTER")["BY_FLOOR"]
    groups = [g for f in scopes.values() for g in f["GROUPS"]]
    assert groups and all(g["ANSWER_PROPAGATES"].startswith("only as far as") for g in groups)


def test_the_before_and_after_table_covers_every_defect_this_review_found(rec):
    rows = rec["BEFORE_AND_AFTER"]["DEFECTS_THIS_REVIEW_FOUND"]
    assert [r["REVIEW_POINT"] for r in rows] == [str(i) for i in range(1, 11)]
    for r in rows:
        assert r["DEFECT"] and r["EVIDENCE_FOR_THE_CHANGE"]
        assert r["R6_BEFORE"] is not None and r["R7_AFTER"] is not None
        assert r["AUTOMATIC_OR_QUESTION"] in ("AUTOMATIC", "REMAINS_A_ROOT_QUESTION")


def test_every_number_in_the_report_is_a_declared_claim_that_agrees_with_its_register(rec):
    claims = rec["REPORT_CLAIMS"]
    assert claims["ALL_DECLARED_CLAIMS_AGREE"] and claims["CLAIM_COUNT"] >= 10
    for c in claims["CLAIMS"]:
        assert c["VERIFIED"] is True, c
        assert c["REGISTER_PATH"] or c["QUERY"]
    prose = rec["GENERATED_PROSE"]
    assert prose["SENTENCES"]
    assert "all prose verified" in prose["WHAT_THIS_DOES_NOT_PROVE"]


def test_the_release_sentences_come_from_the_simulation_and_not_from_a_count_of_edges(rec):
    sim = rec["ANSWER_IMPACT_SIMULATION"]
    for scenario in sim["BY_KIND"]:
        assert scenario["RELEASES_NODES"] + scenario["STILL_BLOCKED"] >= scenario["RELEASES_NODES"]
        if scenario["STILL_BLOCKED"]:
            assert scenario["WHAT_REMAINS"], scenario
    per = {x["ROOT_QUESTION_ID"]: x for x in sim["PER_QUESTION"]}
    assert per
    for rec_q in per.values():
        assert rec_q["RELEASES_NODES"] <= rec_q["AFFECTS_NODES"]


def test_the_evidence_version_is_the_same_in_every_derived_register(rec):
    lineage = register("EVIDENCE_VERSION_AND_DERIVATION_LINEAGE")["BY_FLOOR"]
    for floor, row in lineage.items():
        assert set(row["DERIVED_REGISTERS"].values()) == {row["EVIDENCE_VERSION"]}, floor
        assert row["BARRIER"]["UNCHANGED"] is True, floor
...
def test_nothing_on_this_drawing_is_excluded_from_the_trade_at_all(rec):
    """R6 excluded twenty-three bands for being shorter than twice their thickness, on WEAK confidence.

    Under R7 an exclusion needs positive evidence - the source naming a column, a proven duplicate, a
    junction already inside the walls that cross there.  This source offers none of those for those bands,
    so all twenty-three are questions instead, and nothing leaves the trade silently.
    """
    excluded = rec["FINAL_VERSUS_BLOCKED"]["EXCLUDED_ROWS"]
    assert excluded == [], excluded
    by_floor = register("GEOMETRY_EXCLUSION_EVIDENCE_REGISTER")["BY_FLOOR"]
    short = [x for f in by_floor.values() for x in f["SHORT_BUT_NOT_EXCLUDED"]]
    assert short, "the drawing does contain bands shorter than twice their thickness"
    for row in short:
        assert row["ASPECT_LENGTH_OVER_THICKNESS"] < MA.WALL_ASPECT_MIN
        assert row["WHY"]
...
def test_the_independent_acceptance_gate_passes_on_every_floor(rec):
    ...
    for floor, g in rec["ACCEPTANCE_GATE"]["BY_FLOOR"].items():
        assert g["OF"] == 31 and g["PASSED"] == 31, (floor, g["BLOCKERS"])
        assert {c["CHECK"] for c in g["CHECKS"]} == {f"A{i}" for i in range(1, 32)}
...
def test_the_before_and_after_table_says_what_changed_and_why(rec):
    ...
    assert ba["R6_COLUMN_READ_FROM"]
    for row in ba["OPENINGS"]:
        assert row["R6_CLASSIFICATION_BEFORE"] and row["R7_CLASSIFICATION"]
