...
from agents.base import ModelFn, extract_json, ladder, run_json_agent
...
def run_grid(payload: GridBrief, model: ModelFn | None = None) -> GridDesign:
    ...
    # Nine tiles of bilingual copy is a long answer; give the ladder room.
    models = [model] if model else ladder(max_tokens=24000)
    last: Exception | None = None
    for fn in models:
        try:
            return parse(extract_json(fn(system, user)))
...
def run(payload: PostBrief, model: ModelFn | None = None) -> PostDesign:
    ...
    return run_json_agent(PROMPT_PATH, user, PostDesign.from_dict, model=model)
