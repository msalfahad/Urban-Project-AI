"""Assemble ALRASHED_7_GENERIC_ENGINE_VALIDATION.zip, and verify it after it is written.
...
from research.qs_wall_treatment_01.pa09.alrashed import regression_r7 as RG   # noqa: E402

OUT = Path(PR.OUT_DIR) / "pa09_alrashed"
REPORTS = ROOT / "data" / "reports"
ZIP = REPORTS / "ALRASHED_7_GENERIC_ENGINE_VALIDATION.zip"

ENGINE = sorted((ROOT / "engine" / "qs_core").glob("*.py"))
ADAPTER = ROOT / "research/qs_wall_treatment_01/pa09/alrashed/regression_r7.py"
TESTS = sorted((ROOT / "tests").glob("test_qs_core_*.py")) + \
    [ROOT / "tests/test_pa09_alrashed_regression_r7.py",
...
REGISTERS = [
    "ALRASHED_GENERIC_ENGINE_VALIDATION_R7",
    "ALRASHED_FINAL_OPENING_STATE_REGISTER",
    "ALRASHED_EVIDENCE_VERSION_AND_DERIVATION_LINEAGE",
    "ALRASHED_WINDOW_ROOM_AND_CATEGORY_REGISTER",
    "ALRASHED_OPENING_AREA_AND_DEDUCTION_REGISTER",
    "ALRASHED_ROOT_QUESTION_REGISTER",
    "ALRASHED_DEPENDENCY_IMPACT_REGISTER",
    "ALRASHED_BLOCKER_SET_AND_ANSWER_IMPACT_REGISTER",
    "ALRASHED_MATERIAL_SCOPE_REGISTER",
    "ALRASHED_GEOMETRY_EXCLUSION_EVIDENCE_REGISTER",
    "ALRASHED_REPORT_CLAIM_REGISTER",
    "ALRASHED_ACCEPTANCE_GATE_R7",
...
def run_tests():
    ...
    header = [
        ...
        "#   python -m pytest tests/test_qs_core_*.py tests/test_pa09_alrashed_regression_r7.py "
        "-o addopts= -v",
    ...
    suites = [("the engine, its failure cases and the independent gate",
              ...
              ("the real-drawing regression",
               ["-o", "addopts=", "-v", "-p", "no:randomly",
                "tests/test_pa09_alrashed_regression_r7.py",
...
def before_after_markdown(rec):
    ...
    out = ["# Every defect this review found, before and after", "",
           f"The R6 column is read from `{ba['R6_COLUMN_READ_FROM']}` (sha256 "
           f"`{ba.get('R6_PACKAGE_SHA256')}`), not from disk, so it can be checked independently.  No "
           "expected quantity and no expected question count appears anywhere below: every AFTER value is "
           "read from the register this run produced.", ""]
    for r in ba.get("DEFECTS_THIS_REVIEW_FOUND", []):
        out += [f"## {r['REVIEW_POINT']}. {r['DEFECT']}", "",
                f"- **R6:** `{json.dumps(r['R6_BEFORE'], default=str)}`",
                f"- **R7:** `{json.dumps(r['R7_AFTER'], default=str)}`",
                f"- **Why it moved:** {r['EVIDENCE_FOR_THE_CHANGE']}",
                f"- **{r['AUTOMATIC_OR_QUESTION']}**", ""]
    out += ["## Objects whose admission class changed", ""]
    if not ba.get("OPENINGS"):
        out += [ba.get("OPENINGS_NOTE", "none"), ""]
    else:
        out += ["| object | floor | R6 | R7 | source evidence | automatic or question |",
                "|---|---|---|---|---|---|"]
        for r in ba["OPENINGS"]:
            out.append(f"| {r['OBJECT']} | {r['FLOOR']} | {r['R6_CLASSIFICATION_BEFORE']} | "
                       f"{r['R7_CLASSIFICATION']} | {r['SOURCE_EVIDENCE_FOR_THE_CHANGE']} | "
                       f"{r['AUTOMATIC_OR_QUESTION']} |")
        out.append("")
    out += ["## What an answer is actually worth", "",
            "Generated from the blocker-set simulation, not written beside it.", "",
            "| answer this | releases | still blocked afterwards |", "|---|---|---|"]
    for s_ in rec["ANSWER_IMPACT_SIMULATION"]["BY_KIND"]:
        out.append(f"| {s_['SCENARIO']} ({s_['QUESTIONS_ANSWERED']}) | {s_['RELEASES_NODES']} | "
                   f"{s_['STILL_BLOCKED']} |")
    everything = rec["ANSWER_IMPACT_SIMULATION"]["ANSWERING_EVERYTHING"]
    out += [f"| answer everything | {everything['RELEASES_NODES']} | {everything['STILL_BLOCKED']} |", "",
            rec["ANSWER_IMPACT_SIMULATION"]["RULE"], ""]
    return "\n".join(out)


def claims_markdown(rec):
    claims = rec["REPORT_CLAIMS"]
    out = ["# Every number this round reports, and the register that decides it", "",
           claims["WHAT_THIS_PROVES"] + ".", "",
           "**This does not prove** " + claims["WHAT_THIS_CHECK_DOES_NOT_PROVE"], "",
           "| claim | statement | kind | register path | value | register says | agrees |",
           "|---|---|---|---|---|---|---|"]
    for c in claims["CLAIMS"]:
        out.append(f"| {c['CLAIM_ID']} | {c['STATEMENT'].format(value=c['REGISTER_SAYS'])} | "
                   f"{c['CLAIM_TYPE']} | `{c['REGISTER_PATH'] or c['QUERY']}` | {c['VALUE']} | "
                   f"{c['REGISTER_SAYS']} | {'yes' if c['VERIFIED'] else 'NO'} |")
    out += ["", "## The paragraph these generate", ""]
    for line in rec["GENERATED_PROSE"]["SENTENCES"]:
        out.append(f"- {line}")
    return "\n".join(out)
...
def build():
    ...
    (staging / "REPORT_CLAIMS.md").write_text(claims_markdown(rec), "utf-8")
    print("running the suites")
    (staging / "TEST_LOG.txt").write_text(run_tests(), "utf-8")
    for name in ("ROOT_QUESTIONS.md", "FINAL_VERSUS_BLOCKED.md", "BEFORE_AND_AFTER.md",
                 "REPORT_CLAIMS.md", "TEST_LOG.txt"):
        entries[name] = staging / name

    packaging_commit = git("rev-parse", "HEAD")
    manifest = {
        "PACKAGE": ZIP.name,
        "WHAT": "the reusable takeoff engine after the R7 corrections - one evidence barrier every register is "
                "derived from, questions asked of the final state, dependencies as idempotent blocker "
                "sets, material answers scoped explicitly, exclusions on positive evidence only, and "
                "every reported number declared as a checkable claim - with its adapter, the "
        ...
        "RESULT": {
            ...
            "EVIDENCE_BARRIER_HOLDS": all(f["EVIDENCE_BARRIER"]["UNCHANGED"] for f in rec["FLOORS"]),
            "DUPLICATE_IMPACT_ROWS": rec["ROOT_QUESTIONS_AND_IMPACTS"]["DUPLICATE_IMPACT_ROWS"],
            "DECLARED_CLAIMS_ALL_AGREE": rec["REPORT_CLAIMS"]["ALL_DECLARED_CLAIMS_AGREE"],
    ...
    (REPORTS / "ALRASHED_7_PACKAGE_VERIFICATION.json").write_text(
