...
def render(register: dict) -> dict:
    ...
    for t in register["TRACES"]:
        geom = next((g for g in P.TRACE_GEOMETRY_FIELDS if t.get(g)), None)
        if not geom:
            undrawable.append({"TRACE_ID": t.get("TRACE_ID"),
                               "CLAIM_TYPE": t.get("CLAIM_TYPE"),
                               "VISUAL_TRACE_STATUS":
                                   t.get("VISUAL_TRACE_STATUS"),
                               "WHY_NOT_DRAWN":
                                   "the trace carries no geometry, so there "
                                   "is nowhere honest to draw it"})
            continue
        by_sheet.setdefault((t["CASE_ID"], t["SHEET_ID"]), []).append((t, geom))
    ...
    for (case_id, sheet_id), traces in sorted(by_sheet.items()):
        ...
        for t, geom in traces:
            ...
            elif geom == "PIXEL_POINT":
                x, y = g
                r = 16
                pts = [(x - r, y - r), (x + r, y - r), (x + r, y + r),
                       (x - r, y + r), (x - r, y - r)]
                anchor = (x + r + 4, y - r)
            elif geom == "PIXEL_POLYGON":
                pts = [tuple(p) for p in g] + [tuple(g[0])]
                anchor = (min(p[0] for p in g), min(p[1] for p in g) - 34)
            else:
                pts = [tuple(p) for p in g]
                anchor = (pts[0][0], pts[0][1] - 34)
            _dashed(d, pts, st["dash"], st["width"], st["rgb"])
            _label(d, anchor,
        ...
        ly = 14
        d.rectangle([10, 10, 470, 24 + 30 * (len(legend) + 1)],
